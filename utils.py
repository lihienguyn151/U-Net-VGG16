...
def unique_mask_values(idx, mask_dir, mask_suffix):
    ...
    if mask.ndim == 2:
        ...
        raise ValueError(f'Lihie_Nguyn>>> Loaded masks should have 2 or 3 dimensions, found {mask.ndim}')
...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0, mask_suffix: str = '',
        ...
        assert 0 < scale <= 1, 'Lihie_Nguyn>>> Scale must be between 0 and 1'
        self.scale = scale
        ...
        if not self.ids:
            raise RuntimeError(
                f'Lihie_Nguyn>>> No input file found in {images_dir}, make sure you put your images there')
    ...
    def preprocess(mask_values, pil_img, scale, is_mask):
        w, h = pil_img.size
        newW, newH = 512, 512  #Scaling images
...
def plot_confusion_matrix(y_true, y_pred, classes, save_path):
    cm = confusion_matrix(y_true.flatten(), y_pred.flatten(), labels=range(len(classes)))
